...
See [API Reference](../4.reference/1.api.md#options) for complete API documentation.
...
Monitor `Stats().QueueDepths` to detect signals approaching capacity. See [Architecture](../1.learn/4.architecture.md) for details on per-signal isolation.
...
[Flux](https://github.com/zoobz-io/flux) is a reactive configuration library that watches files for changes and invokes callbacks when they update. Integrate it with capitan for file-based hot-reload:
