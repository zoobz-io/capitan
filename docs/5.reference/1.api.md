...
Applies per-signal configuration to the default instance. Replaces any previous config entirely. See [Configuration Guide](../3.guides/1.configuration.md#per-signal-configuration) for details.
...
The `Signals` map keys can be exact signal names or glob patterns (`*`, `?`, `[...]`). See [Configuration Guide](../3.guides/1.configuration.md#per-signal-configuration) for resolution rules.
