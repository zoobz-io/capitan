...
**[Observability](4.cookbook/1.observability.md)** - Forward events to your existing structured logging. Filter by severity, correlate by request context.

**[Workflows](4.cookbook/2.workflows.md)** - A `payment.succeeded` listener emits `order.fulfilled`, which triggers `inventory.reserved` and `notification.queued`.

**[Distribution](4.cookbook/3.distribution.md)** - Bridge capitan to external message brokers. Consume events from Kafka or NATS, process them locally, emit results back to the distributed system.

**[Persistence](4.cookbook/4.persistence.md)** - Store events to a database for audit trails, debugging, and replay. Re-emit historical events through the same signal infrastructure.
