...
Events are pooled via `sync.Pool` - never store event pointers. See [Architecture - Event Pooling](../1.learn/3.architecture.md#event-pooling) for details.
...
- [Event Patterns](./2.patterns.md) - Common event coordination patterns
- [Concurrency Guide](./4.concurrency.md) - Thread-safe event handling
